...
$(LUA_LIB_BUILT): $(LUA_SRC_DIR_UNPACKED) $(LUA_INCLUDE_DIR_MADE)
	...
	cd lua/$(LUA_NAME) && make -e TO_LIB="liblua.a liblua.so liblua.so.$(LUA_VERSION)" INSTALL_DATA='cp -d' INSTALL_TOP='$(LUA_BUILD_DIR)' install
	cp $(LUA_BUILD_DIR)/bin/* $(PWD)/bin/
	cp $(LUA_BUILD_DIR)/include/*.h $(LUA_INCLUDE_DIR)
	cp $(LUA_BUILD_DIR)/lib/*.so* $(LUA_LIBDIR)
